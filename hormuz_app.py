...
with st.sidebar:
    ...
    st.markdown("<div style='font-size:0.62rem;color:#4a5a75;text-transform:uppercase;"
                ...
                unsafe_allow_html=True)
    ais_seconds = st.slider("Secondi raccolta", 20, 120, 45, 5)
    only_tankers = st.checkbox("Solo tanker/cargo", value=True)

    bbox_choice = st.radio("Area geografica", ["Stretto Hormuz", "Golfo Persico completo"],
                           label_visibility="collapsed")
    bbox = HORMUZ_BBOX if bbox_choice == "Stretto Hormuz" else GULF_BBOX
...
if "Mappa" in page:
    top_bar("🗺️ Mappa Live — Stretto di Hormuz",
            f"<span class='dot-live'></span>Dati AIS in tempo reale via AISStream.io")

    # Pulsante raccolta
    col_btn, col_info = st.columns([1, 3])
    with col_btn:
        run_ais = st.button("📡 Aggiorna navi", use_container_width=True)
    with col_info:
        st.markdown(f"<div style='font-size:0.75rem;color:#4a5a75;padding-top:0.6rem;'>"
                    f"Raccoglie dati AIS per {ais_seconds} secondi nell'area selezionata. "
                    f"Ogni punto sulla mappa è una nave reale.</div>",
                    unsafe_allow_html=True)

    if run_ais:
        with st.spinner(f"Connessione AISStream.io... raccolta per {ais_seconds}s"):
            df_v = fetch_ais_snapshot(api_key, ais_seconds, bbox, only_tankers)
        st.session_state.vessels_df   = df_v
        st.session_state.last_ais_time = datetime.now().strftime("%H:%M:%S")
        st.rerun()

    if st.session_state.vessels_df is not None:
        df_v = st.session_state.vessels_df

        if df_v.empty:
            st.warning("Nessuna nave rilevata nell'area. Prova ad aumentare i secondi di raccolta "
                       "o a selezionare 'Golfo Persico completo'.")
        else:
            # ── KPI ──────────────────────────────────────────────────────────
            n_tot     = len(df_v)
            n_tanker  = int(df_v["is_tanker"].sum()) if "is_tanker" in df_v.columns else n_tot
            avg_speed = df_v["speed"].dropna().mean() if "speed" in df_v.columns else 0
            n_moving  = int((df_v["speed"].fillna(0) > 1).sum()) if "speed" in df_v.columns else 0

            k1, k2, k3, k4 = st.columns(4)
            for col, (lbl, val, sub, c) in zip([k1,k2,k3,k4], [
                ("Navi rilevate",  str(n_tot),         f"area {bbox_choice}",        "#3b82f6"),
                ("Tanker/Cargo",   str(n_tanker),       "tipo nave confermato",       "#8b5cf6"),
                ("In movimento",   str(n_moving),       f"velocità > 1 nodo",         "#10b981"),
                ("Velocità media", f"{avg_speed:.1f} kn","speed over ground",          "#06b6d4"),
            ]):
                with col: kpi(lbl, val, sub, c)

            # ── MAPPA ────────────────────────────────────────────────────────
            section("POSIZIONI NAVI IN TEMPO REALE")

            # Colori per tipo nave
            def vessel_color(row):
                if row.get("is_tanker", False):
                    spd = row.get("speed", 0) or 0
                    if spd < 1:   return "#f59e0b"   # ambra = ferma/ancorata
                    elif spd < 8: return "#ef4444"   # rosso = lenta
                    else:         return "#10b981"   # verde = in transito normale
                return "#3b82f6"   # blu = cargo/altro

            df_v["color"] = df_v.apply(vessel_color, axis=1)
            ...
            def hover_text(row):
                ...
                dest = row.get("destination","") or "n/a"
                typ  = row.get("ship_type_label","") or "Unknown"
                nav  = row.get("nav_status_label","") or "Unknown"
                return (f"<b>{name}</b><br>"
                        f"MMSI: {row['mmsi']}<br>"
                        f"Tipo: {typ}<br>"
                        f"Velocità: {spd}<br>"
                        f"Destinazione: {dest}<br>"
                        f"Status: {nav}")

            df_v["hover"] = df_v.apply(hover_text, axis=1)

            fig_map = go.Figure()

            # Layer 1: stretto di Hormuz highlight
            fig_map.add_scattermapbox(
                lat=[26.4, 26.4, 26.8, 26.8, 26.4],
                lon=[55.8, 57.0, 57.0, 55.8, 55.8],
                mode="lines",
                line=dict(color="rgba(6,182,212,0.4)", width=2),
                name="Stretto Hormuz",
                hoverinfo="skip",
            )
            ...
            fig_map.add_scattermapbox(
                ...
                mode="markers",
                marker=dict(
                    size=df_v["size"].tolist(),
                    color=df_v["color"].tolist(),
                    opacity=0.85,
                ),
            ...
            fig_map.update_layout(
                ...
                legend=dict(
                    ...
                    bordercolor="#1e2d47",
                    font=dict(color="#8a9ab5", size=10),
                    x=0.01, y=0.99,
                ),
            )
            ...
            # Legenda colori
            st.markdown("""
            <div style='font-size:0.68rem;color:#4a5a75;display:flex;gap:1.5rem;margin-top:0.3rem;'>
                <span>🟢 In transito (>8 kn)</span>
                <span>🔴 Lenta (1-8 kn)</span>
                <span>🟡 Ferma/Ancorata (<1 kn)</span>
                <span>🔵 Cargo/Altro</span>
            </div>""", unsafe_allow_html=True)

            # ── Tabella navi ─────────────────────────────────────────────────
            section("DETTAGLIO NAVI RILEVATE")

            show_cols = ["name","mmsi","ship_type_label","speed","heading",
                         "nav_status_label","destination","lat","lon"]
            show_cols = [c for c in show_cols if c in df_v.columns]
            df_show   = df_v[show_cols].copy()
            df_show.columns = [c.replace("_label","").replace("_"," ").title()
                               for c in show_cols]
            if "Speed" in df_show.columns:
                df_show["Speed"] = df_show["Speed"].apply(
            ...
            baseline_low, baseline_high = 13, 23
            delta = n_tanker - 18
            delta_pct = delta / 18 * 100
            ...
            with col_g:
                fig_gauge = go.Figure(go.Indicator(
                    ...
                    gauge={
                        ...
                        "bar":   {"color": get_disruption_color(
                                    "CRITICAL" if n_tanker < 10 else
                                    "ELEVATED" if n_tanker < 14 else "NORMAL")},
            ...
            with col_t:
                level_now = ("CRITICAL" if n_tanker < 10 else
                             "ELEVATED" if n_tanker < 14 else "NORMAL")
                color_now = get_disruption_color(level_now)
                emoji_now = get_disruption_emoji(level_now)

                st.markdown(f"""
                <div style='padding:1rem;'>
                    <div style='font-size:0.68rem;color:#4a5a75;text-transform:uppercase;
                                letter-spacing:0.1em;margin-bottom:0.5rem;'>Lettura traffico</div>
                    <div class='level-badge-{level_now}'>{emoji_now} {level_now}</div>
                    <div style='margin-top:1rem;font-size:0.8rem;color:#8a9ab5;line-height:1.8;'>
                        Navi rilevate: <b style='color:#e8edf5;'>{n_tanker}</b><br>
                        Baseline storica: <b style='color:#e8edf5;'>18 navi</b><br>
                        Delta: <b style='color:{color_now};'>{delta:+d} ({delta_pct:+.1f}%)</b><br>
                        Velocità media: <b style='color:#e8edf5;'>{avg_speed:.1f} kn</b>
                    </div>
                    <div style='margin-top:1rem;font-size:0.72rem;color:#4a5a75;'>
                        Nota: la baseline di 18 navi è una stima su dati storici aggregati.
                        Con storico AIS persistente la baseline sarà calcolata dinamicamente.
                    </div>
                </div>""", unsafe_allow_html=True)

    else:
        # Stato iniziale — mostra mappa vuota con solo il bounding box
        st.markdown("""<div style='background:rgba(6,182,212,0.05);border:1px solid rgba(6,182,212,0.2);
            ...
            <div style='font-size:0.78rem;color:#4a5a75;'>
                Clicca "Aggiorna navi" per connetterti ad AISStream.io e visualizzare
                le navi in tempo reale nello Stretto di Hormuz.
            </div>
        </div>""", unsafe_allow_html=True)

        # Mappa vuota con solo il contorno dell'area
        fig_empty = go.Figure()
        fig_empty.add_scattermapbox(
            lat=[21.0, 21.0, 27.0, 27.0, 21.0],
            lon=[54.0, 60.0, 60.0, 54.0, 54.0],
            mode="lines",
