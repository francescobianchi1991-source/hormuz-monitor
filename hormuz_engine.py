...
def fetch_ais_snapshot(
    ...
    # AIS SOG: alcuni ricevitori inviano in decimi di nodo (es. 485 = 48.5 -> 4.85 kn)
    # Un tanker non supera mai 20 nodi -> se speed > 30 probabilmente è in decimi
    df.loc[df["speed"] > 30, "speed"] = df.loc[df["speed"] > 30, "speed"] / 10.0
    # Rimuovi valori ancora impossibili (>25 kn per navi commerciali)
    df.loc[df["speed"] > 25, "speed"] = np.nan
